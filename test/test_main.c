...
static void t_meta_rng(tk_ctx *tk)
{
    ...
    for (i = 0; i < 1000; i++) {
        TK_CHECK(tk, tk_rng_range(&a, UINT64_C(1) << 63) < (UINT64_C(1) << 63));
        TK_CHECK(tk, tk_rng_range(&a, UINT64_MAX) < UINT64_MAX);
    }
}
...
static const tk_module *const modules[] = {
    ...
    &tk_module_sp_arith,
    &tk_module_modular,
    &tk_module_monty_review,
    &tk_module_primality,
    &tk_module_aprcl_review,
    &tk_module_tinyprp_review,
    &tk_module_sieve,
    &tk_module_microecm,
    &tk_module_tinyecm,
#ifdef TK_WITH_LAYER3
